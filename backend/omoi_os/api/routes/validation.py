...
class GiveReviewRequest(BaseModel):
    ...
    validator_agent_id: str = Field(..., description="ID of validator agent submitting review")
    ...
    recommendations: Optional[list[str]] = Field(None, description="Optional list of recommendations")
...
class SpawnValidatorRequest(BaseModel):
    ...
    commit_sha: Optional[str] = Field(None, description="Optional Git commit SHA for validation")
...
class ValidationStatusResponse(BaseModel):
    ...
    review_done: bool = Field(..., description="Whether latest validation cycle completed successfully")
...
def get_validation_orchestrator() -> ValidationOrchestrator:
    ...
    if not validation_orchestrator:
        raise HTTPException(
            status_code=503,
            detail="Validation orchestrator service not available"
        )
...
def spawn_validator(
    ...
    if not validator_id:
        ...
        if status is None:
            ...
            raise HTTPException(
                status_code=409,
                detail="validator_already_running"
            )
