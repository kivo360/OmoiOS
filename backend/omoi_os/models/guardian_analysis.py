...
from sqlalchemy import Boolean, DateTime, Float, Integer, Interval, String, Text, ForeignKey, Index
...
class GuardianAnalysis(Base):
    ...
    steering_type: Mapped[Optional[str]] = mapped_column(
        String(64), nullable=True
    )
    steering_recommendation: Mapped[Optional[str]] = mapped_column(
        Text, nullable=True
    )
    trajectory_summary: Mapped[str] = mapped_column(
        Text, nullable=False
    )
    last_claude_message_marker: Mapped[Optional[str]] = mapped_column(
        Text, nullable=True
    )
    accumulated_goal: Mapped[Optional[str]] = mapped_column(
        Text, nullable=True
    )
    ...
    conversation_length: Mapped[Optional[int]] = mapped_column(
...
class ConductorAnalysisModel(Base):
    ...
    num_agents: Mapped[int] = mapped_column(
    ...
    duplicates: Mapped[list["DetectedDuplicateModel"]] = relationship(
        "DetectedDuplicateModel", back_populates="conductor_analysis", cascade="all, delete-orphan"
    )
...
class DetectedDuplicateModel(Base):
    ...
    agent1_id: Mapped[str] = mapped_column(
        String(255), nullable=False, index=True
    )
    agent2_id: Mapped[str] = mapped_column(
        String(255), nullable=False, index=True
    )
    similarity_score: Mapped[float] = mapped_column(
        Float, nullable=False, index=True
    )
    work_description: Mapped[Optional[str]] = mapped_column(
        Text, nullable=True
    )
    ...
    __table_args__ = (
        Index('idx_duplicates_agents', 'agent1_id', 'agent2_id'),
    )
...
class SteeringInterventionModel(Base):
    ...
    steering_type: Mapped[str] = mapped_column(
    ...
    __table_args__ = (
        Index('idx_steering_agent_created', 'agent_id', 'created_at'),
    )
