...
class AgentBaseline(Base):
    ...
    latency_ms: Mapped[float] = mapped_column(
        Float, nullable=False, default=0.0, comment="Baseline latency in milliseconds"
    )
    latency_std: Mapped[float] = mapped_column(
        Float, nullable=False, default=1.0, comment="Standard deviation of latency"
    )
    error_rate: Mapped[float] = mapped_column(
        Float, nullable=False, default=0.0, comment="Baseline error rate (0-1)"
    )
    cpu_usage_percent: Mapped[float] = mapped_column(
        Float, nullable=False, default=0.0, comment="Baseline CPU usage percentage"
    )
    memory_usage_mb: Mapped[float] = mapped_column(
        Float, nullable=False, default=0.0, comment="Baseline memory usage in MB"
    )
    ...
    additional_metrics: Mapped[Optional[Dict[str, Any]]] = mapped_column(
        JSONB,
        nullable=True,
        comment="Additional baseline metrics (e.g., active_connections, pending_operations)",
    )
