...
class MCPTool(Base):
    ...
    schema: Mapped[Dict[str, Any]] = mapped_column(
        JSONB, nullable=False, comment="JSON Schema for tool"
    )
    ...
    __table_args__ = (
        UniqueConstraint("server_id", "tool_name", name="uq_mcp_tools_server_tool"),
    )
...
class CircuitBreakerState(Base):
    ...
    circuit_key: Mapped[str] = mapped_column(
        String(255), primary_key=True, comment="server_id:tool_name"
    )
    state: Mapped[str] = mapped_column(
        String(20), nullable=False, comment="CLOSED, OPEN, HALF_OPEN"
    )
...
class MCPInvocation(Base):
    ...
    result_summary: Mapped[Optional[str]] = mapped_column(
        Text, nullable=True, comment="Redacted result"
    )
    ...
    policy_decision: Mapped[Optional[str]] = mapped_column(
        String(10), nullable=True, comment="ALLOW/DENY"
    )
    cached_decision: Mapped[bool] = mapped_column(
