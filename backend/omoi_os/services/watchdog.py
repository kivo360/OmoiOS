...
class WatchdogService:
    ...
    DETECTION_TIME_THRESHOLD = timedelta(
        seconds=20
    )  # Detect unresponsiveness within 20s
    ...
    def __init__(
        ...
        if policies_dir is None:
            ...
            policies_dir = (
                Path(__file__).parent.parent.parent / "config" / "watchdog_policies"
            )
            ...
            try:
                with open(policy_file, "r") as f:
                    ...
                    if policy_data and "policy" in policy_data:
                        policy_name = policy_data["policy"].get(
                ...
                logger.error(
                    "Error loading policy", policy_file=str(policy_file), error=str(e)
                )
        ...
        default_policies = {
            "monitor_restart.yaml": {
                "policy": {
                    ...
                    "actions": [
                        {"type": "restart", "priority": 1, "timeout_seconds": 60}
                    ],
                    "escalation": {
        ...
        with self.db.get_session() as session:
            ...
            for action in sorted(actions, key=lambda a: a.get("priority", 999)):
                ...
                if action_type == "restart":
                    success = self._execute_restart(
                        agent_id, watchdog_action.id, reason
                    )
                elif action_type == "failover":
                    success = self._execute_failover(
                        agent_id, watchdog_action.id, action
                    )
                elif action_type == "escalate":
                    success = self._execute_escalation(
                        agent_id, watchdog_action.id, reason
                    )
                else:
                    # Unknown action type
    ...
    def _execute_restart(
        ...
        try:
            ...
            logger.error(
                "Error executing restart for agent",
                agent_id=agent_id,
                error=str(e),
                exc_info=True,
            )
    ...
    def _escalate_to_guardian(
        ...
        try:
            ...
            logger.error(
                "Error escalating to Guardian for agent",
                agent_id=agent_id,
                error=str(e),
                exc_info=True,
            )
    ...
    def get_remediation_history(
        ...
        with self.db.get_session() as session:
            ...
            actions = (
                query.order_by(WatchdogAction.created_at.desc()).limit(limit).all()
            )
