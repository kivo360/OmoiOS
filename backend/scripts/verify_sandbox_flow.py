...
async def main():
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                if isinstance(tasks, list) and len(tasks) > 0:
                    for t in tasks:
                        if t.get("ticket_id") == ticket_id or (not task_id):
                            ...
                            if conversation_id:
                                print(f"   ✅ Conversation registered: {conversation_id}")
                            break
        ...
        if not task_id:
            print("⚠️  No task found yet (orchestrator may be slow or no LLM configured)")
        ...
        print("   (Check terminal 11.txt for 'Daytona sandbox' and 'register_conversation')")
        ...
        print(f"{'✅' if conversation_id else '⚠️ '} Conversation registered: {conversation_id or 'Not yet'}")
        
        print("\n🔍 To manually verify sandbox spawning, check:")
        print(f"   Terminal logs: Look for 'Daytona sandbox' and 'omoios-*'")
        print(f"   MCP calls: Look for 'register_conversation' and 'report_agent_event'")
