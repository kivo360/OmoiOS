    ...
    try:
        ...
        try:
            ...
            for sb in sandboxes:
                ...
                if sb.id == sandbox_id or task_id == sandbox_id or sandbox_id in str(sb.id):
                    target_sandbox = sb
                    break
            
            if not target_sandbox:
                print(f"Sandbox not found.")
                return
        
        print(f"Found sandbox: {target_sandbox.id} (state: {getattr(target_sandbox, 'state', 'unknown')})")
        
        # Get the worker log
        print("\n=== Worker Log (/tmp/worker.log) ===")
        try:
            result = target_sandbox.process.exec("cat /tmp/worker.log 2>/dev/null || echo '[No log file yet]'")
        ...
        try:
            result = target_sandbox.process.exec("ps aux | grep -E 'sandbox_worker|python' | grep -v grep || echo '[No python process]'")
            print(result.result if hasattr(result, 'result') else result)
        except Exception as e:
        ...
        try:
            result = target_sandbox.process.exec("cat /tmp/.sandbox_env 2>/dev/null | head -20 || echo '[No env file]'")
...
def list_sandboxes():
    ...
    try:
        ...
        sandboxes = getattr(result, 'items', result) if hasattr(result, 'items') else list(result)
