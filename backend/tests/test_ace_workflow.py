...
from omoi_os.services.ace_reflector import Reflector, ReflectorResult, Error, Insight
from omoi_os.services.ace_curator import Curator, CuratorResult, DeltaOperation, PlaybookDelta, PlaybookBullet
...
class TestReflector:
    ...
    def test_search_playbook_entries(
        ...
        with db_service.get_session() as session:
            entry = PlaybookEntry(
                ...
                embedding=embedding_service.generate_embedding("Always use JWT tokens for authentication"),
                is_active=True,
                created_at=utc_now(),
                updated_at=utc_now(),
            )
...
class TestCurator:
    ...
    def test_validate_delta_rejects_duplicates(
        ...
        with db_service.get_session() as session:
            current_playbook = session.query(PlaybookEntry).filter(
    ...
    def test_search_playbook_for_similar(
        ...
        with db_service.get_session() as session:
            entry = PlaybookEntry(
                ...
                content="Always validate JWT token expiration",
                category="best_practices",
                embedding=embedding_service.generate_embedding("Always validate JWT token expiration"),
                is_active=True,
                created_at=utc_now(),
                updated_at=utc_now(),
            )
...
class TestACEEngine:
    ...
    def test_execute_workflow_complete_flow(
        self, db_service, memory_service, embedding_service, event_bus_service, test_task, test_ticket
    ):
    ...
    def test_execute_workflow_with_errors(
        self, db_service, memory_service, embedding_service, event_bus_service, test_task
    ):
        ...
        assert any(e["error_type"] == "ImportError" for e in result_obj.errors_identified)
    ...
    def test_execute_workflow_creates_playbook_entry(
        ...
        with db_service.get_session() as session:
            entry = session.query(PlaybookEntry).filter(
                PlaybookEntry.ticket_id == test_ticket,
                PlaybookEntry.supporting_memory_ids.contains([result_obj.memory_id]),
            ).first()
