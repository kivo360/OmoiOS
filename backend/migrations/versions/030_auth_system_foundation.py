...
def upgrade() -> None:
    ...
    safe_add_column(
        "users",
        sa.Column(
            "is_super_admin", sa.Boolean(), server_default="false", nullable=False
        ),
    )
    ...
    if column_exists("users", "email_confirmed_at") and column_exists(
        "users", "is_verified"
    ):
        print("📊 Migrating existing user data...")
        op.execute(
            "UPDATE users SET is_verified = (email_confirmed_at IS NOT NULL) WHERE is_verified = false"
        )
    ...
    safe_create_table(
        ...
        sa.Column(
            "org_attributes", postgresql.JSONB(astext_type=sa.Text()), nullable=True
        ),
        sa.Column(
            "max_concurrent_agents", sa.Integer(), server_default="5", nullable=False
        ),
        sa.Column(
            "max_agent_runtime_hours",
            sa.Float(),
        ...
        sa.ForeignKeyConstraint(
            ["owner_id"], ["users.id"], name="fk_organizations_owner"
        ),
    ...
    safe_create_table(
        ...
        sa.ForeignKeyConstraint(
            ["inherits_from"], ["roles.id"], name="fk_roles_parent", ondelete="SET NULL"
        ),
        sa.PrimaryKeyConstraint("id"),
    ...
    safe_create_index(
        "idx_org_role_name", "roles", ["organization_id", "name"], unique=True
    )
    ...
    safe_create_table(
        ...
        sa.ForeignKeyConstraint(
            ["user_id"],
            ["users.id"],
            name="fk_org_memberships_user",
            ondelete="CASCADE",
        ),
        sa.ForeignKeyConstraint(
            ["agent_id"],
            ["agents.id"],
            name="fk_org_memberships_agent",
            ondelete="CASCADE",
        ),
        sa.ForeignKeyConstraint(
            ["organization_id"],
            ["organizations.id"],
            name="fk_org_memberships_org",
            ondelete="CASCADE",
        ),
        sa.ForeignKeyConstraint(
            ["role_id"], ["roles.id"], name="fk_org_memberships_role"
        ),
        sa.ForeignKeyConstraint(
            ["invited_by"], ["users.id"], name="fk_org_memberships_inviter"
        ),
        sa.PrimaryKeyConstraint("id"),
    ...
    safe_create_index(
        "idx_org_memberships_user", "organization_memberships", ["user_id"]
    )
    safe_create_index(
        "idx_org_memberships_agent", "organization_memberships", ["agent_id"]
    )
    safe_create_index(
        "idx_org_memberships_org", "organization_memberships", ["organization_id"]
    )
    safe_create_index(
        "idx_user_org",
        "organization_memberships",
    ...
    safe_create_index(
        "idx_sessions_token_hash", "sessions", ["token_hash"], unique=True
    )
    ...
    safe_create_table(
        ...
        sa.ForeignKeyConstraint(
            ["agent_id"], ["agents.id"], name="fk_api_keys_agent", ondelete="CASCADE"
        ),
        sa.ForeignKeyConstraint(
            ["organization_id"],
            ["organizations.id"],
    ...
    safe_create_foreign_key(
        "fk_projects_creator", "projects", "users", ["created_by"], ["id"]
    )
