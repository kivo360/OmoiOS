...
def upgrade() -> None:
    ...
    if "watchdog_actions" not in existing_tables:
        ...
        op.create_index(
            "ix_watchdog_actions_target_agent_id",
            "watchdog_actions",
            ["target_agent_id"],
        )
        op.create_index(
        ...
        op.create_index(
            "ix_watchdog_actions_guardian_action_id",
            "watchdog_actions",
            ["guardian_action_id"],
        )
        ...
        existing_indexes = [
            idx["name"] for idx in inspector.get_indexes("watchdog_actions")
        ]
...
def downgrade() -> None:
    ...
    op.drop_index(
        "ix_watchdog_actions_guardian_action_id", table_name="watchdog_actions"
    )
