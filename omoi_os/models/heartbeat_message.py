...
class HeartbeatMessage(BaseModel):
    ...
    timestamp: datetime = Field(default_factory=utc_now, description="ISO8601 timestamp")
    sequence_number: int = Field(..., description="Monotonically increasing sequence number")
    status: str = Field(..., description="Agent status: idle, running, degraded, failed")
    current_task_id: Optional[str] = Field(None, description="UUID of current task or null")
    health_metrics: Dict[str, Any] = Field(
        default_factory=dict,
        description="Health metrics: cpu_usage_percent, memory_usage_mb, active_connections, etc.",
    )
    checksum: str = Field(..., description="SHA256 checksum of payload for integrity validation")
...
class HeartbeatAck(BaseModel):
    ...
    timestamp: datetime = Field(default_factory=utc_now, description="ISO8601 timestamp of ack")
    received: bool = Field(True, description="Whether heartbeat was successfully processed")
    message: Optional[str] = Field(None, description="Optional message (warnings, errors, etc.)")
