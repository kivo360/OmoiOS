...
def test_resource_reallocation_policy_exists():
    ...
    assert policy_file.exists(), f"Resource reallocation policy not found: {policy_file}"
...
def test_policy_evaluator_rollback_enabled():
    ...
    for policy_filename in ["emergency.yaml", "resource_reallocation.yaml", "priority_override.yaml"]:
        policy_file = POLICY_DIR / policy_filename
        evaluator = PolicyEvaluator(policy_file)
