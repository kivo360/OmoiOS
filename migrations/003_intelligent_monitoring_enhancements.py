...
def upgrade() -> None:
    ...
    op.create_table('guardian_analyses',
    ...
    op.create_index('idx_guardian_analyses_agent_created', 'guardian_analyses', ['agent_id', 'created_at'])
    op.create_index('idx_guardian_analyses_alignment', 'guardian_analyses', ['alignment_score'])
    op.create_index('idx_guardian_analyses_needs_steering', 'guardian_analyses', ['needs_steering'])

    # Conductor system analyses table
    op.create_table('conductor_analyses',
        sa.Column('id', sa.UUID(), nullable=False),
    ...
    op.create_index('idx_conductor_analyses_created', 'conductor_analyses', ['created_at'])
    op.create_index('idx_conductor_analyses_coherence', 'conductor_analyses', ['coherence_score'])
    op.create_index('idx_conductor_analyses_status', 'conductor_analyses', ['system_status'])

    # Detected duplicates table
    op.create_table('detected_duplicates',
        sa.Column('id', sa.UUID(), nullable=False),
    ...
    op.create_index('idx_duplicates_cycle', 'detected_duplicates', ['conductor_analysis_id'])
    op.create_index('idx_duplicates_agents', 'detected_duplicates', ['agent1_id', 'agent2_id'])
    op.create_index('idx_duplicates_similarity', 'detected_duplicates', ['similarity_score'])

    # Steering interventions table
    op.create_table('steering_interventions',
        sa.Column('id', sa.UUID(), nullable=False),
        sa.Column('agent_id', sa.String(length=255), nullable=False),
    ...
    op.create_index('idx_steering_agent_created', 'steering_interventions', ['agent_id', 'created_at'])
    ...
    op.create_table('monitoring_vectors',
        sa.Column('id', sa.UUID(), nullable=False),
        ...
        sa.Index('idx_monitoring_vectors_embedding', 'embedding', postgresql_using='gin')
    )
    op.create_index('idx_monitoring_vectors_entity', 'monitoring_vectors', ['entity_type', 'entity_id'])
    op.execute('CREATE INDEX idx_monitoring_vectors_embedding ON monitoring_vectors USING gin (embedding)')

    # Monitoring audit log for compliance
    op.create_table('monitoring_audit_log',
        sa.Column('id', sa.UUID(), nullable=False),
    ...
    op.create_index('idx_monitoring_audit_created', 'monitoring_audit_log', ['created_at'])
    op.create_index('idx_monitoring_audit_event_type', 'monitoring_audit_log', ['event_type'])

    # Add monitoring configuration to agents table
    op.add_column('agents', 'anomaly_score', sa.Float(), nullable=True, default=0.0)
    op.add_column('agents', 'consecutive_anomalous_readings', sa.Integer(), nullable=True, default=0)
