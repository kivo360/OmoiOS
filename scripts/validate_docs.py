...
def validate_filename(file: Path) -> List[str]:
    ...
    if re.search(r'[A-Z]', name) and not name.isupper():
        errors.append(f"{Colors.RED}❌ {file.relative_to('docs')}: Use snake_case, not CamelCase{Colors.NC}")
    
    # Check for hyphens (except ADRs with numbers)
    if '-' in name and not re.match(r'^\d{3}_', name):
        errors.append(f"{Colors.RED}❌ {file.relative_to('docs')}: Use underscores (_), not hyphens (-){Colors.NC}")
    
    # Check for spaces
    if ' ' in name:
        errors.append(f"{Colors.RED}❌ {file.relative_to('docs')}: No spaces in filenames{Colors.NC}")
...
def validate_metadata(file: Path) -> List[str]:
    ...
    try:
        ...
        errors.append(f"{Colors.RED}❌ {file.relative_to('docs')}: Error reading file: {e}{Colors.NC}")
