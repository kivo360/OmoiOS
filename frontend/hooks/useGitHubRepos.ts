...
export const githubRepoKeys = {
  ...
  repos: (params?: ListReposParams) =>
    [...githubRepoKeys.all, "list", params] as const,
  repo: (owner: string, repo: string) =>
    [...githubRepoKeys.all, "detail", owner, repo] as const,
  ...
  return useMutation<BranchCreateResult, Error, CreateBranchRequest>({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: githubRepoKeys.branches(owner, repo),
      });
    },
  });
}
 ...
 */
export function useGitHubDirectory(
  owner: string,
  repo: string,
  path: string = "",
  ref?: string,
...
export function useCreateOrUpdateFile(owner: string, repo: string) {
  ...
  >({
    ...
    onSuccess: (_, { path, data }) => {
      ...
      queryClient.invalidateQueries({
        queryKey: githubRepoKeys.file(owner, repo, path),
      });
      ...
      queryClient.invalidateQueries({
        queryKey: githubRepoKeys.tree(owner, repo, "HEAD"),
      });
      queryClient.invalidateQueries({
        queryKey: githubRepoKeys.commits(owner, repo),
      });
    },
  });
}
...
export function useCreatePullRequest(owner: string, repo: string) {
  ...
  return useMutation<PullRequestCreateResult, Error, CreatePullRequestRequest>({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: githubRepoKeys.pullRequests(owner, repo),
      });
    },
  });
}
