  ...
  const handleEvent = useCallback(
    (event: BoardEvent) => {
      ...
      if (
        ...
        event.event_type === "SANDBOX_SPAWNED"
      ) {
        const taskId = event.entity_id;
        const ticketId = (event.payload.ticket_id as string) || "";
        const sandboxId =
          (event.payload.sandbox_id as string) ||
          (event.entity_type === "sandbox" ? event.entity_id : undefined);
      ...
      if (
        ...
        event.event_type === "SANDBOX_COMPLETED"
      ) {
        setRunningTasks((prev) => {
          const next = new Map(prev);
        ...
        case "TICKET_PHASE_ADVANCED":
          ...
          if (event.entity_id) {
            queryClient.invalidateQueries({
              queryKey: ticketKeys.detail(event.entity_id),
            });
          }
          break;
