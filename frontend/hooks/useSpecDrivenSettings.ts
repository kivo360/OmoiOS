...
// Default settings
export const DEFAULT_SETTINGS: Omit<SpecDrivenSettings, "id" | "user_id" | "created_at" | "updated_at"> = {
  auto_execute: true,
  execution_mode: "auto",
...
export function getSettingsWarnings(settings: Partial<SpecDrivenSettings>): SettingsWarning[] {
  const warnings: SettingsWarning[] = []

  if (settings.auto_merge && settings.validation_mode === "none") {
    warnings.push({
      field: "auto_merge",
      message: "Auto-merge with no validation is risky. Consider enabling validation.",
      severity: "warning",
    })
  }

  if (settings.coverage_threshold !== undefined && settings.coverage_threshold < 50 && settings.enforce_coverage) {
    warnings.push({
  ...
  if (settings.parallel_execution && settings.max_parallel_tasks !== undefined && settings.max_parallel_tasks > 5) {
    warnings.push({
...
export function validateSettings(settings: Partial<SpecDrivenSettings>): ValidationError[] {
...
export function useUpdateSpecDrivenSettings() {
  ...
  return useMutation({
    mutationFn: (data: SpecDrivenSettingsUpdate) => updateSpecDrivenSettings(data),
    onSuccess: (data) => {
