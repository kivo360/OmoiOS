...
export const useOnboardingStore = create<OnboardingState>()(
  persist(
    (set, get) => ({
      ...
      setCompletedChecklistItems: (items) =>
        set({ completedChecklistItems: items }),

      addCompletedChecklistItem: (item) =>
        set((state) => ({
      ...
      syncFromServer: (serverState) =>
        set({
          ...
          completedChecklistItems:
            serverState.completed_checklist_items as ChecklistItemId[],
...
export function useOnboarding() {
  ...
  useEffect(() => {
    if (
      !data.firstSpecId ||
      data.firstSpecStatus === "completed" ||
      data.firstSpecStatus === "failed"
    ) {
      return;
    }
  ...
  const submitFirstSpec = useCallback(
        ...
        if (!projectId && data.selectedRepo) {
          if (!organizationId) {
            const orgResponse = await api
              .post<{ id: string }>("/api/v1/organizations", {
                name: `${user?.full_name || "My"}'s Workspace`,
                slug: user?.email?.split("@")[0] || "workspace",
              })
              .catch(async () => {
                const orgs = await api.get<Array<{ id: string }>>(
          ...
          const projectResponse = await api.post<{ id: string }>(
            "/api/v1/projects",
            {
