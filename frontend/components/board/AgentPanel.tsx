...
export function AgentPanel({
  ...
  const { events, isConnected, connectionError, isLoadingHistory, refresh } =
  ...
  const currentStatus = useMemo(() => {
    ...
    if (
      lastEvent?.event_type === "agent.error" ||
      lastEvent?.event_type === "agent.failed"
    )
      return "failed";
    return "running";
  }, [events]);
    ...
    <div className="h-full flex flex-col bg-background border-l">
      ...
      <div className="flex items-center justify-between p-3 border-b bg-muted/30">
        <div className="flex items-center gap-2 min-w-0 flex-1">
          <StatusIcon
            className={`h-4 w-4 shrink-0 ${iconClass} text-${statusColor}`}
          />
      ...
      <div className="flex items-center gap-2 px-3 py-2 border-b text-xs">
        <Badge
          variant={
            statusColor as "default" | "secondary" | "destructive" | "outline"
          }
        >
          {statusLabel}
        </Badge>
