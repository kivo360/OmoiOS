...
export default function WorkspacePage({ params }: WorkspacePageProps) {
  ...
  const { data: commitsData, isLoading: commitsLoading } =
    useAgentCommits(agentId);
  const [selectedCommit, setSelectedCommit] = useState<CommitDisplay | null>(
    null,
  );
        ...
        <div className="mt-4 flex items-start justify-between">
          <div>
            ...
            <p className="text-muted-foreground mt-1">
              {agent.phase_id?.replace("PHASE_", "") ||
                "Agent workspace files and commits"}
            </p>
          </div>
        ...
        <TabsContent value="settings" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Workspace settings and metadata</CardDescription>
            </CardHeader>
            <CardContent>
              <Table>
                <TableBody>
