...
const statusColor = (status: string) => {
  if (status === "aligned" || status === "active" || status === "idle")
    return "bg-emerald-100 text-emerald-700";
  if (status === "warning" || status === "degraded")
    return "bg-amber-100 text-amber-700";
  if (status === "drifting" || status === "busy")
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="text-right">
          ...
          <p className="text-muted-foreground">
            Alignment monitoring per agent
          </p>
        </div>
      </div>
      ...
      <Card>
        <CardHeader className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <CardDescription>
              Alignment score, drift, and idle detection
            </CardDescription>
          </div>
        ...
        <CardContent className="space-y-3">
          ...
          {isLoading ? (
            ...
            rows.map((row) => (
              <div
                key={row.agentId}
                className="grid grid-cols-12 items-center rounded-lg border px-4 py-3"
              >
                ...
                <div className="col-span-3 space-y-2">
                  <div className="flex items-center gap-2">
                    <Badge
                      className={statusColor(row.rawStatus)}
                      variant="outline"
                    >
                      {row.rawStatus}
                    </Badge>
