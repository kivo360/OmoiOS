...
import { Star, GitPullRequest, ExternalLink, BarChart3, ListChecks, Shield } from "lucide-react"
...
export async function generateMetadata({
  ...
  return {
    ...
    description: data.description || `${data.stats.task_count} tasks automated with OmoiOS`,
    openGraph: {
      title: `${data.title} — Built with OmoiOS`,
      description: data.description || `${data.stats.task_count} tasks automated, ${data.stats.tasks_completed} completed`,
      type: "website",
    },
    twitter: {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#2d2618] via-[#1a150d] to-[#0f0c08] text-white">
      <div className="mx-auto max-w-2xl px-6 py-16">
        {/* Header */}
        <div className="mb-2 text-sm font-medium text-amber-400/80">OmoiOS Showcase</div>
        <h1 className="text-3xl font-bold tracking-tight">{data.title}</h1>
        {data.description && (
          <p className="mt-3 text-base text-white/60 leading-relaxed">{data.description}</p>
        )}
        {data.project_name && (
          <p className="mt-1 text-sm text-white/40">Project: {data.project_name}</p>
        )}
        ...
        {data.pull_request_url && (
          ...
          >
            ...
            <span className="flex-1 text-sm">Pull Request #{data.pull_request_number}</span>
        ...
        <p className="mt-16 text-center text-xs text-white/30">
          Built with{" "}
          <a href="https://omoios.dev" className="underline hover:text-white/50">
            OmoiOS
          </a>{" "}
