 ...
 */
export async function moveTicket(data: MoveTicketRequest): Promise<{
  ticket_id: string;
  new_phase: string;
  new_column: string;
  status: string;
 ...
 */
export async function checkWIPViolations(
  projectId?: string,
): Promise<WIPViolation[]> {
  const url = projectId
 ...
 */
export async function getColumnForPhase(phaseId: string): Promise<{
  id: string;
  name: string;
