...
export async function searchAgents(params?: {
  ...
  const url = query ? `/api/v1/agents/search?${query}` : "/api/v1/agents/search"
 ...
 * Get stale agents
 */
export async function getStaleAgents(timeoutSeconds?: number): Promise<Agent[]> {
  const url = timeoutSeconds
...
export async function cleanupStaleAgents(params?: {
  ...
  return apiRequest<{ success: boolean; marked_count: number; message: string }>(
    url,
    { method: "POST" }
  )
}
