...
describe("SpecDrivenSettingsPanel", () => {
  ...
  describe("Loading State", () => {
    it("renders loading skeleton when data is loading", () => {
      ...
      expect(
        screen.getByText("Configure your spec-driven development workflow"),
      ).toBeInTheDocument();
    });
  });
  ...
  describe("Panel Display After Load", () => {
    ...
    it("displays all settings after successful load", () => {
      ...
      expect(
        screen.getByLabelText(/Automatic - Execute specs immediately/),
      ).toBeInTheDocument();
      expect(
        screen.getByLabelText(/Manual - Review specs before execution/),
      ).toBeInTheDocument();
      ...
      expect(
        screen.getByTestId("parallel-execution-switch"),
      ).toBeInTheDocument();
      ...
      expect(
        screen.getByTestId("notify-completion-switch"),
      ).toBeInTheDocument();
    ...
    it("displays correct initial values from settings", () => {
      ...
      const autoRadio = screen.getByLabelText(
      ...
      const enforceCoverageSwitch = screen.getByTestId(
        "enforce-coverage-switch",
      );
  ...
  describe("Radio Group Selection", () => {
    ...
    it("changes validation mode", async () => {
      ...
      const strictRadio = screen.getByLabelText(
        /Strict - All validations must pass/,
      );
      const relaxedRadio = screen.getByLabelText(
  ...
  describe("Warning Banners", () => {
    it("shows warning when auto-merge is enabled with no validation", async () => {
      ...
      expect(
        screen.getByText(/Auto-merge with no validation is risky/),
      ).toBeInTheDocument();
    });
    ...
    it("shows warning when auto-merge is enabled without required tests", async () => {
      ...
      expect(
        screen.getByText(
          /Auto-merge without required tests could introduce bugs/,
        ),
      ).toBeInTheDocument();
    });
    ...
    it("shows warning for low coverage threshold", async () => {
      ...
      expect(
        screen.getByText(
          /Coverage threshold below 50% may result in low-quality code/,
        ),
      ).toBeInTheDocument();
    });
  });
  ...
  describe("Unsaved Changes Dialog", () => {
    ...
    it("does not show dialog when closing without changes", async () => {
      render(<SpecDrivenSettingsPanel onClose={mockOnClose} />, {
